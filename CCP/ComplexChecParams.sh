...
files=$(find $PROG_DIR/rdt -type f | wc -l )
if [[ $files == 0 ]]; then
    echo ""
    echo "Файлов rdt в папке $PROG_DIR/rdt не найдено."
    exit 0
fi
ls $PROG_DIR/rdt | column -t > $PROG_DIR/tmp/files_list.tmp

for ((i=1; i<=$files; i++)); do
    ind="NR==$i"
    name=$(ls $PROG_DIR/rdt | column -t | awk $ind)
    vers=$(cat $PROG_DIR/rdt/$name | grep vers -i | grep -oE '[0-9]*\.?[0-9]+')
    prot=$(cat $PROG_DIR/rdt/$name | grep "VER_PROTOCOL=" | grep -oE '[0-9]+')
    [[ -z $prot ]] && prot=0
    prot="Протокол:$prot"
    Application=$(cat $PROG_DIR/rdt/$name | head -n 1)
    if echo "$Application" | grep -wq "Application"; then
        vers=$(cat $PROG_DIR/rdt/$name | grep "ApplVersion" | grep -oE '[0-9]*\.[0-9]*\.[0-9]*')
        prot="Не_поддерживается"
    fi
     echo -e "$name $vers $prot" >> $PROG_DIR/tmp/list1.tmp
done

cat $PROG_DIR/tmp/list1.tmp | column -t >> $PROG_DIR/tmp/list2.tmp
list=$(cat $PROG_DIR/tmp/list2.tmp | nl -s ' -> ')
...
for ((i=1; i<=$files; i++)); do
 ...
 NAME_FILE=$(cat $PROG_DIR/tmp/files_list.tmp | awk $NUM_FILE)
 rm $PROG_DIR/tmp/* 2>/dev/null
 clear
# КОНЕЦ МЕНЮ ВЫБОРА ФАЙЛА

# Путь к обрабатываемому файлу
TARGET="$PROG_DIR/rdt/$NAME_FILE"
...
if [[ "$P_NAME_FILE" == 3 ]] && [[ $NAME_FILE =~ ^[a-zA-Z] ]]; then
    devnum=$(echo "$NAME_FILE" | sed 's/.*_//' | cut -d'.' -f1)
fi
if [[ "$P_NAME_FILE" == 3 ]] && [[ "$NAME_FILE" =~ ^[0-9] ]]; then
...
if [ -z "$device_id" ]; then
    ...
    echo "------------------------------------------------------------------------" >> $LOG
    echo "" >> $LOG
    echo "[$DATE_STR][$TIME_STR][$MODULE_NAME][I][Скопирован файл: $TARGET в $IN_DIR]" >> $LOG
    echo "Скопирован файл: $TARGET в $IN_DIR"
    while [ -f "$IN_DIR/$NAME_FILE" ]; do
        echo "Обработка файла $NAME_FILE ..."
        sleep 1 # Пауза в 1 секунду
        while [ -f "$IN_DIR/$NAME_FILE" ]; do
            if [ -n "$(find "$IN_DIR/$NAME_FILE" -type f -mmin +0,3 2>/dev/null)" ]; then
                TIME_STR=$(date +"%H:%M:%S")
                echo "[$DATE_STR][$TIME_STR][$MODULE_NAME][F][Файл $NAME_FILE не обрабатвыается!]" >> $LOG
                echo "Файл $NAME_FILE не обрабатвыается!"
                exit 1
            fi
        done
    done
    ...
    # удаление записи
    echo "Перезапись в БД..."
    export PGPASSWORD=$Password
    psql -U $Login -h $Host -d $Name -p $Port -tA -c "DELETE FROM devices_custs.device  WHERE devnum = '$devnum';" > /dev/null 2>&1
    unset PGPASSWORD
    echo "" >> $LOG
    echo "" >> $LOG
    echo "------------------------------------------------------------------------" >> $LOG
    echo "" >> $LOG
    TIME_STR=$(date +"%H:%M:%S")
    echo "[$DATE_STR][$TIME_STR][$MODULE_NAME][I][Удалена запись из БД прибора: ${devnum}" >> $LOG
    echo "Удалена запись из БД прибора: ${devnum}"
    # обработка файла
    cp $TARGET $IN_DIR
    TIME_STR=$(date +"%H:%M:%S")
    echo "[$DATE_STR][$TIME_STR][$MODULE_NAME][I][Скопирован файл: $TARGET в $IN_DIR]" >> $LOG
...
where device_id=$device_id and attribute_name='SIM_ACTIV';")
unset PGPASSWORD
export PGPASSWORD=$Password
...
echo -e "id прибора: ${GREEN}$device_id${NC}"
...
echo "[$DATE_STR][$TIME_STR][$MOD][$MODULE_NAME][id прибора: $device_id]" >> $LOG
...
if [ -z "$SIM_ACTIV" ]; then
    ...
    where ds.device_id=$device_id and attribute_id=54 and sim_num=$SIM_ACTIV;")
    unset PGPASSWORD
fi
APN_ADDRESS=${arr[$i]}
...
if [ -z "$SIM_ACTIV" ]; then
    ...
    join dicts.attributes_dict as ad on ad.id=si.attribute_id
    join devices_custs.device_sim as ds on ds.id=si.sim_id
    where ds.device_id=$device_id and attribute_id=52 and sim_num=$SIM_ACTIV;")
    unset PGPASSWORD
fi
APN_LOGIN=${arr[$i]}
...
if [ -z "$SIM_ACTIV" ]; then
    ...
    where device_id=$device_id and attribute_id=53;")
    unset PGPASSWORD
else
    # Получения значения из БД
    export PGPASSWORD=$Password
    ...
    join dicts.attributes_dict as ad on ad.id=si.attribute_id
    join devices_custs.device_sim as ds on ds.id=si.sim_id
    where ds.device_id=$device_id and attribute_id=53 and sim_num=$SIM_ACTIV;")
    unset PGPASSWORD
fi
APN_PASSWORD=${arr[$i]}
...
if [ -z "$SIM_ACTIV" ]; then
    ...
    where device_id=$device_id and attribute_id=55;")
    unset PGPASSWORD
else
    # Получения значения из БД
    export PGPASSWORD=$Password
    ...
    join dicts.attributes_dict as ad on ad.id=si.attribute_id
    join devices_custs.device_sim as ds on ds.id=si.sim_id
    where ds.device_id=$device_id and attribute_id=55 and sim_num=$SIM_ACTIV;")
    unset PGPASSWORD
fi
TCP_ADDRESS=${arr[$i]}
...
if [ -z "$SIM_ACTIV" ]; then
    ...
    join dicts.attributes_dict as ad on ad.id=si.attribute_id
    join devices_custs.device_sim as ds on ds.id=si.sim_id
    where ds.device_id=$device_id and attribute_id=41 and sim_num=$SIM_ACTIV;")
    unset PGPASSWORD
fi
BALANCE_PHONE=${arr[$i]}
...
if [ -z "$SIM_ACTIV" ]; then
    ...
    join dicts.attributes_dict as ad on ad.id=si.attribute_id
    join devices_custs.device_sim as ds on ds.id=si.sim_id
    where ds.device_id=$device_id and attribute_id=42 and sim_num=$SIM_ACTIV;")
    unset PGPASSWORD
fi
MODE_TRANSFER=${arr[$i]}
...
join dicts.attributes_dict on dicts.attributes_dict.id = info_params.device_info_params.attribute_id
where device_id=$device_id and attribute_id=57;")
unset PGPASSWORD
...
join dicts.attributes_dict on dicts.attributes_dict.id = info_params.device_info_params.attribute_id
where device_id=$device_id and attribute_id=59;")
unset PGPASSWORD
SENSOR_TEMP=${arr[$i]}
...
if [ -z "$SIM_ACTIV" ]; then
    ...
    join dicts.attributes_dict as ad on ad.id=si.attribute_id
    join devices_custs.device_sim as ds on ds.id=si.sim_id
    where ds.device_id=$device_id and attribute_id=99 and sim_num=$SIM_ACTIV;")
    unset PGPASSWORD
fi
...
if [ -z "$SIM_ACTIV" ]; then
    ...
    join dicts.attributes_dict as ad on ad.id=si.attribute_id
    join devices_custs.device_sim as ds on ds.id=si.sim_id
    where ds.device_id=$device_id and attribute_id=44 and sim_num=$SIM_ACTIV;")
    unset PGPASSWORD
fi
SEANCECNT=${arr[$i]}
...
if [ -z "$SIM_ACTIV" ]; then
    ...
    join dicts.attributes_dict as ad on ad.id=si.attribute_id
    join devices_custs.device_sim as ds on ds.id=si.sim_id
    where ds.device_id=$device_id and attribute_id=45 and sim_num=$SIM_ACTIV;")
    unset PGPASSWORD
fi
SEANCECNT_ERR=${arr[$i]}
...
if [ -z "$SIM_ACTIV" ]; then
    ...
    join dicts.attributes_dict as ad on ad.id=si.attribute_id
    join devices_custs.device_sim as ds on ds.id=si.sim_id
    where ds.device_id=$device_id and attribute_id=101 and sim_num=$SIM_ACTIV;")
    unset PGPASSWORD
fi
AUTOSWITH=${arr[$i]}
...
if [ -z "$SIM_ACTIV" ]; then
    ...
    where device_id=$device_id and attribute_id=2669;")
    unset PGPASSWORD
else
    # Получения значения из БД
    export PGPASSWORD=$Password
    ...
    join dicts.attributes_dict as ad on ad.id=si.attribute_id
    join devices_custs.device_sim as ds on ds.id=si.sim_id
    where ds.device_id=$device_id and attribute_id=2669 and sim_num=$SIM_ACTIV;")
    unset PGPASSWORD
fi
CNT_FAIL_SIM=${arr[$i]}
...
if [ -z "$SIM_ACTIV" ]; then
    ...
    where device_id=$device_id and attribute_id=56;")
    unset PGPASSWORD
else
    export PGPASSWORD=$Password
    ...
    join dicts.attributes_dict as ad on ad.id=si.attribute_id
    join devices_custs.device_sim as ds on ds.id=si.sim_id
    where ds.device_id=$device_id and attribute_id=56 and sim_num=$SIM_ACTIV;")
    unset PGPASSWORD
fi
TCP_ADDRESS2=${arr[$i]}
...
if [ -n "$VERS_K" ]; then
    if (( $(echo "$VERS_K >= 1.050299" | bc -l) )); then
        ...
        join dicts.attributes_dict as ad on ad.id=si.attribute_id
        join devices_custs.device_sim as ds on ds.id=si.sim_id
        where ds.device_id=$device_id and attribute_id=2672 and sim_num=$SIM_ACTIV;")
        unset PGPASSWORD
...
where device_id=$device_id and attribute_id=2675;")
unset PGPASSWORD
...
if [ -n "$VERS_K" ]; then
    if (( $(echo "$VERS_K >= 1.050299" | bc -l) )); then
        ...
        where device_id=$device_id and attribute_id=2676;")
        unset PGPASSWORD
        CNT_REZREP=${arr[$i]}
...
where device_id=$device_id and attribute_id=2677;")
unset PGPASSWORD
...
where device_id=$device_id and attribute_id=2679;")
unset PGPASSWORD
...
where device_id=$device_id and attribute_id=2686;")
unset PGPASSWORD
SIM_ENABLE=${arr[$i]}
...
where device_id=$device_id and attribute_id=2680;")
unset PGPASSWORD
SIM_ACTIV=${arr[$i]}
...
where device_id=$device_id and attribute_id=2681;")
unset PGPASSWORD
...
where device_id=$device_id and attribute_id=2682;")
unset PGPASSWORD
