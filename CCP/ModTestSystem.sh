...
sleep 0.15
TIME_STR=$(date +"%H:%M:%S")
FILE_SGS_JSON=$(find /opt -type f -name "sgs.json")
...
if [ "$STATUS_DBS" = "active" ]; then
    echo -e "\e[1;32m●\e[0m SGS_AUP_DbWriterService запущен"
    echo "[$DATE_STR][$TIME_STR][$MOD][$MODULE_NAME][Passed][SGS_AUP_DbWriterService запущен]" >> $LOG
    echo "DbWriterStatus = 0" >> $timestamp
else
    echo -e "\e[1;31m●\e[0m SGS_AUP_DbWriterService не запущен"
    echo "[$DATE_STR][$TIME_STR][$MOD][$MODULE_NAME][Failed][SGS_AUP_DbWriterService не запущен]" >> $LOG
...
STATUS_DBS=$(systemctl is-active "$SERVICE_NAME")
if [ "$STATUS_DBS" = "active" ]; then
    echo -e "\e[1;32m●\e[0m SGS_AUP_SMTHandlerService запущен"
    echo "[$DATE_STR][$TIME_STR][$MOD][$MODULE_NAME][Passed][SGS_AUP_SMTHandlerService запущен]" >> $LOG
    echo "SmtHandlerStatus = 0" >> $timestamp
else
    echo -e "\e[1;31m●\e[0m SGS_AUP_SMTHandlerService не запущен"
    echo "[$DATE_STR][$TIME_STR][$MOD][$MODULE_NAME][Failed][SGS_AUP_SMTHandlerService не запущен]" >> $LOG
...
STATUS_DBS=$(systemctl is-active "$SERVICE_NAME")
if [ "$STATUS_DBS" = "active" ]; then
    echo -e "\e[1;32m●\e[0m SGS_AUP_ValidatorService запущен"
    echo "[$DATE_STR][$TIME_STR][$MOD][$MODULE_NAME][Passed][SGS_AUP_ValidatorService запущен]" >> $LOG
    echo "ValidatorStatus = 0" >> $timestamp
else
    echo -e "\e[1;31m●\e[0m SGS_AUP_ValidatorService не запущен"
    echo "[$DATE_STR][$TIME_STR][$MOD][$MODULE_NAME][Failed][SGS_AUP_ValidatorService не запущен]" >> $LOG
...
STATUS_DBS=$(systemctl is-active "$SERVICE_NAME")
if [ "$STATUS_DBS" = "active" ]; then
    echo -e "\e[1;32m●\e[0m SGS_AUP_WatcherService запущен"
    echo "[$DATE_STR][$TIME_STR][$MOD][$MODULE_NAME][Passed][SGS_AUP_WatcherService запущен]" >> $LOG
    echo "WatcherStatus = 0" >> $timestamp
else
    echo -e "\e[1;31m●\e[0m SGS_AUP_WatcherService не запущен"
    echo "[$DATE_STR][$TIME_STR][$MOD][$MODULE_NAME][Failed][SGS_AUP_WatcherService не запущен]" >> $LOG
